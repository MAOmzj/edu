# 文件用途：提供网页静态资源、问答、流式回答、会话历史和删除数据的 FastAPI 接口。
# 调用关系：Uvicorn 启动本文件的 app；本文件调用 memory/runtime.py 中的共享 Agent 与存储。
# 修改易踩坑：阻塞型 Agent 要放在线程池，流接口只能发送最终答案，接口不能泄露内部 State 或密钥。
"""小学教育知识问答系统 Web 服务。
