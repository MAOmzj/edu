# 文件用途：封装 Tavily 搜索、联网条件判断、隐私脱敏、可信域名限制和本地回退。
# 调用关系：subject_agent.py 默认注册，review_agent.py 可选注册；本文件按需调用 Tavily 和本地 RAG。
# 修改易踩坑：不得上传姓名/会话/长期记忆，非时效问题必须先本地检索，任何异常都要安全回退。
